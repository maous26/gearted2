...
const extraProducts: MockProduct[] = Array.from({ length: 40 }).map((_, i) => {
...
let mockProducts: MockProduct[] = [...BASE_PRODUCTS, ...extraProducts];
...
export function findMockProductById(id: string): MockProduct | undefined {
  return mockProducts.find((product) => product.id === id);
}

export function addMockProduct(product: Omit<MockProduct, 'id' | 'createdAt'> & Partial<Pick<MockProduct, 'id' | 'createdAt'>>): MockProduct {
...
export function resetMockProducts(products: MockProduct[]): void {
